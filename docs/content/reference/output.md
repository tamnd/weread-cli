...
describes what users get. Pick a format with `-o`, or let weread choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
weread <command> -o table   # aligned columns for reading
weread <command> -o jsonl   # one JSON object per line, for piping
weread <command> -o json    # a single JSON array
weread <command> -o csv     # spreadsheet friendly
weread <command> -o tsv     # tab-separated
weread <command> -o url     # just the URL column
weread <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
weread <command> --fields id,title,url
```
...
```bash
weread <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
weread <command>            # a table, because this is a terminal
weread <command> | wc -l    # JSONL, because this is a pipe
```
